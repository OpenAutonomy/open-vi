...
| 1.2.6.8 | Receive Vehicle State Data | Supported | Activity, `MA_PositionReportDetailed`, `WeatherObservation`, `NavigationReport`, `ComponentStatus` from `get_vehicle_state()`. Duration is emitted when the port has it. Fuel mass is the backend (PX4 vehicle TOML); Isolator does not invent it. |
...
| NavigationReport | out | Supported | Percent from `get_vehicle_state()`. Duration when the port has it. Fuel mass is the backend (PX4 vehicle TOML). |
